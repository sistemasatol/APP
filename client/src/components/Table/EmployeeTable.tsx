...
import SearchInput from "../Global_Components/SearchInput"; // Importe o componente de pesquisa
import EditEmployee from "../Form/Edit/EditEmployee"; // Importe o Modal de Edição
import DeleteEmployeeConfirm from "../Delete/DeleteEmployeeConfirm"; // Importe o Modal de Exclusão
...
const EmployeeTable: React.FC = () => {
  ...
  const [filteredData, setFilteredData] = useState<Employee[]>([]); // Estado para os dados filtrados
  ...
  const handleSearch = (searchTerm: string) => {
    ...
    if (searchTerm.trim() === "") {
      ...
      const filtered = data.filter((employee) =>
        ...
        employee.enterprise.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        employee.role.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        employee.work.name.toLowerCase().includes(searchTerm.toLowerCase())
      );
          ...
          {filteredData.length > 0 ? (
            filteredData.map((employee) => (
              <tr key={employee.id} className="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td className="py-3 px-6 text-left">{employee.enterprise.name}</td> 
                <td className="py-3 px-6 text-left">{employee.role.name}</td>
                <td className="py-3 px-6 text-left">{employee.work.name}</td>
